...
const currentWhaleQuery = selector({
  ...
  get: ({ get }) => get(whaleInfoQuery(get(currentWhaleIdState))),
});
...
function CurrentWhaleTypes() {
  ...
  const changeWhale = useRecoilCallback(({ snapshot, set }) => (whaleId) => {
    ...
    <>
      {whale === undefined ? (
        ...
        <>
          ...
          <p>
            Diet: {whale.diet} ({whale.favoriteFood})
          </p>
...
function CurrentWhaleIdValue() {
  ...
  return <span>{whaleId.replace("_", " ")}</span>;
}

const App = () => (
  <RecoilRoot>
    <h1>Recoild App (2 level structure)</h1>
    <Suspense fallback={<div>Loading whale types...</div>}>
      <CurrentWhaleTypes />
      <Suspense
        fallback={
          <div>
            Loading <CurrentWhaleIdValue /> info...
          </div>
        }
      >
        <div>
